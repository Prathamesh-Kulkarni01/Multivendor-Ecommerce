import {StyleSheet, Image, TouchableOpacity, View, StatusBar, Text, FlatList} from "react-native";
...
const renderHTMLContent = (htmlContent) => {
    return {__html: "<div><p>This is a sample HTML content.</p></div>"};
};
...
const ProductDetailScreen = ({navigation, route}) => {
    ...
    const handleWishlistBtn = async () => {
        ...
        if (onWishlist) {
            ...
            fetch(`${network.serverip}/remove-from-wishlist?id=${product?._id}`, requestOptions)
            .then((response) => response.json())
            .then((result) => {
                if (result.success) {
            ...
            <View style={styles.bodyContainer}>
                <View style={styles.productImageContainer}>
                    {productImage?.map((img, key) => (
                        <Image key={key} source={{uri: img}} style={styles.productImage} />
                    ))}
                    <FlatList
                        ...
                        renderItem={({item, index}) => (
                            <View style={{margin: 0}} key={index}>
                                <Image key={index} source={{uri: item}} style={styles.productImage} />
                            </View>
                        )}
                    />
                </View>
              ...
              <View style={{display:'flex',flexDirection:'row',overflow:'hidden'}}>
                        <View style={styles.productDetailContainer}>
                            <View style={{paddingStart:20}}>
                                <Text style={styles.secondaryTextSm}>{`SKU: ${product.sku}`}</Text>
                                <Text style={styles.secondaryTextSm}>Category:</Text>
                            </View>
                        </View>
                        ...
                        <View style={styles.productDescriptionContainer}>
                            ...
                            {/* <HTML source={{ html: product?.["Short description"] }} /> */}
                            {/* <Text
...
const styles = StyleSheet.create({
    ...
    productImage: {
        ...
        resizeMode: "contain",
        // resizeMode: "cover",
        mixBlendMode: "multiply", // Example blend mode
    },
    ...
    primaryTextSm: {color: colors.primary, fontSize: 25, fontWeight: "bold"},
